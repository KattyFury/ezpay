// Gọi thẳng Circle Stablecoin Kit API — App Kit SDK không chạy được trên Cloudflare Workers
...
export async function onRequestPost(ctx) {
  ...
  if (action === 'execute') {
    ...
    if (!tx?.target || !tx?.callData) return new Response(JSON.stringify({ error: 'no tx data', detail: swapData }), { status: 500, headers: JSON_HEADERS })
