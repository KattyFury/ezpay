...
- **GOOGLE SIGN-IN** (đang disable ở `Login.jsx`): Circle support trả lời lỗi `155140` do RELOAD trang sau redirect OAuth. Fix: lưu SDK config qua **COOKIES** (không phải sessionStorage) để sống qua redirect — Circle mẫu dùng `cookies-next`. Skill `use-user-controlled-wallets` cũng ghi: "ALWAYS use cookies (not React state/localStorage) for social login flows to persist tokens across OAuth redirects". → khi build: chuyển `ez_google_device*` từ sessionStorage sang cookie, test redirect thật, rồi bật `disabled: true→false`.
- **SWAP execute** (đang disable): làm lại theo chuẩn khi Circle có adapter cho User-Controlled Wallet (hoặc thử route qua contractExecution thủ công). Giao diện Swap cũng cần làm lại theo token design (còn `▾`/`⇅` cũ).
